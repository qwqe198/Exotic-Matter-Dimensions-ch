...
function updateHTML() {
	if (wormholeAnimationActive) {
		if (Date.now() - wormholeAnimationStart > 16000) {
			...
			if (!unlocked("Hawking Radiation")) { wormholeReset(); } // 确保只重置一次
	...
	if (showFooter()) {
		...
		ProgressBar();
		d.innerHTML("span_footerEMDLevel", dictionary(g.EMDLevelDisplayInFooter, { 0: "", 1: " | EMD 等级 " + g.EMDLevel, 2: " | EMD 等级 " + g.EMDLevel + " (分数 " + BEformat(EMDScore()) + ")" }));
	}
	...
	if (g.activeTab === "main") {
		...
		if (g.activeSubtabs.main === "axis") {
			...
			if (g.masteryContainerStyle === "Legacy") {
				for (let i of list) {
					...
					d.innerHTML("span_mastery" + i + "ActiveLegacy", MasteryE(i) ? "激活" : "未激活");
				...
				for (let i of list) {
					...
					d.innerHTML("span_mastery" + i + "ActiveModern", MasteryE(i) ? "激活" : "未激活");
				}
			...
			d.innerHTML("span_overclockCostScaling", (stat.baseOverclockSpeedup > (stat.overclockSoftcap + 1e-10)) ? ("超频成本超过 " + N(stat.overclockSoftcap).noLeadFormat(3) + "× 时显著提高") : "");
			d.innerHTML("button_overclockActive", (timeState === 1) ? "禁用超频" : "启用超频");
			...
			d.innerHTML("button_freezeGame", (timeState === 2) ? "解冻时间" : "冻结时间");
			...
			if (timeAlwaysEqualized()) {
				...
				d.innerHTML("button_equalizeTime", (timeState === 3) ? "正常化时间" : "将所有帧设为 50 毫秒");
			...
			if (g.dilationUpgradesUnlocked > 0) {
				...
				for (let i = 1; i < 5; i++) {
					if (g.dilationUpgradesUnlocked >= i) {
						...
						d.innerHTML("span_dilationUpgrade" + i + "Cost", (g.dilationUpgrades[i] === dilationUpgrades[i].cap) ? "已达上限" : ("成本：" + timeFormat(dilationUpgrades[i].cost()) + " 膨胀时间"));
		...
		if (g.activeSubtabs.options === "options") {
			...
			d.innerHTML("toggleAutosave", g.autosaveIsOn ? "开" : "关");
			d.innerHTML("button_footerDisplay", dictionary(g.footerDisplay, { "All tabs": "在所有标签页显示页脚", "Only Axis tab": "仅在轴子标签页显示页脚", "None": "隐藏页脚" }));
			d.innerHTML("span_newsTickerActive", g.newsTickerActive ? "启" : "禁");
			d.innerHTML("span_newsTickerSpeed", N(g.newsTickerSpeed).noLeadFormat(2));
			d.innerHTML("span_newsTickerDilation", dictionary(g.newsTickerDilation, { "0": "无", "0.0625": "弱", "0.125": "中等", "0.1875": "强", "0.25": "极端" }));
			let doubleClickToBuy = [
				["分配精通", unlocked("Masteries") && (g.masteryContainerStyle === "Modern")],
				["分配星星", unlocked("Stardust") && (g.starContainerStyle === "Modern")],
				["购买研究", unlocked("Hawking Radiation")]
			...
			if (doubleClickToBuy.length === 0) {
				...
				d.innerHTML("button_options_doubleClickToBuy", "双击以" + doubleClickToBuy.map(x => unbreak(x)).join("/") + " " + (g.confirmations.doubleClickToBuy ? "启" : "禁") + "用");
			}
			d.innerHTML("button_options_EMDLevelDisplayInFooter", dictionary(g.EMDLevelDisplayInFooter, { 0: "页脚不显示 EMD 等级", 1: "页脚显示 EMD 等级", 2: "页脚显示 EMD 等级和分数" }));
		...
		if (g.activeSubtabs.statistics === "statistics") {
			...
			for (let i of categories) {
				// 修复1：添加安全检查
				if (breakdownCategories[i] && breakdownCategories[i].contents) {
					let display = false;
					for (let x of breakdownCategories[i].contents) {
						if (miscStats[x] && miscStats[x].visible && miscStats[x].visible()) {
							display = true;
							break;
						}
					}
					...
					d.display("button_SSBnav1_" + i, "none");
				}
			}
			
			// 修复2：检查 activeBreakdownSection 是否有效
			if (activeBreakdownSection && breakdownCategories[activeBreakdownSection] && breakdownCategories[activeBreakdownSection].contents) {
				let contents = breakdownCategories[activeBreakdownSection].contents;
				
				if (contents.length > 1) {
					let lineBreaks = [];
					for (let i of contents) {
						if (miscStats[i] && miscStats[i].newRow) lineBreaks.push(false);
						d.display("button_SSBnav2_" + i, miscStats[i] && miscStats[i].visible && miscStats[i].visible() ? "inline-block" : "none");
						if (miscStats[i] && miscStats[i].visible && miscStats[i].visible() && lineBreaks.length > 0) lineBreaks[lineBreaks.length - 1] = true;
					}
				...
				// 修复3：检查 activeBreakdownTab 是否有效
				let data = miscStats[activeBreakdownTab];
				if (data && data.modifiers && Array.isArray(data.modifiers)) {
			let value;
					for (let i = 0; i < maximumSSBModifierLength; i++) {
						if (i >= data.modifiers.length) {
			...
			for (let i = 1; i < 11; i++) {
				...
				if (wormholeExists) {
					...
					d.innerHTML("span_last10WormholeRuns_notes" + i, [
						["研究 " + roman(Math.max(g.previousWormholeRuns.last10[i - 1].study, 1)), g.previousWormholeRuns.last10[i - 1].study !== 0]
					].filter(x => x[1]).map(x => x[0]).join("; "));
				}
				if (i === 1) {
			...
			for (let i of previousPrestige.buildListNodes) i.innerHTML = previousPrestige.shownBuilds().join("/") + " 套配置";
	...
	if (g.activeTab === "achievements") {
		...
		if (g.activeSubtabs.achievements === "mainAchievements") {
			...
			d.innerHTML("button_achievementToProgressBar", (g.achOnProgressBar === "N") ? "在进度条上显示成就" : "从进度条上隐藏成就");
			...
			d.innerHTML("span_wormholeMilestone18Effect", showFormulas ? (formulaFormat(wormholeMilestone18.formula()) + " 秒") : timeFormat(wormholeMilestone18.eff()));
	...
	if (g.activeTab === "stardust") {
		...
		if (g.activeSubtabs.stardust === "stardustBoosts") {
			...
			for (let i = 1; i < 6; i++) {
				...
				d.innerHTML("span_stardustUpgrade" + i + "Tooltip", (g.stardustUpgrades[i - 1] === stat["stardustUpgrade" + i + "Cap"]) ? (stardustUpgradeNames[i] + " 路径已达上限") : (stardustUpgradeTooltip[i]() + "<br><br>成本：" + stat["stardustUpgrade" + i + "Cost"].format(0) + " 星尘"));
...
d.innerHTML("span_nextStarRow", g.stars >= 40 ? "" : ("你购买的下一个星星将进入第 <span class=\"big _stars\">" + starRow(g.stars + 1) + "</span> 行"));
	...
	if (g.activeTab === "automation") {
		if (StudyE(1)) openTab("wormhole");
		for (let id of Object.keys(autobuyers)) { // 自动购买器相关
			d.display(id + "Autobuyer", autobuyers[id].unlockReq() ? "inline-block" : "none");
			d.innerHTML("span_" + id + "AutobuyerLevel", "等级 " + g[id + "AutobuyerUpgrades"] + " / " + autobuyerMeta.cap(id));
			d.class("button_" + id + "AutobuyerToggle", "automatortoggle " + (g[id + "AutobuyerOn"] ? "on" : "off"));
			d.innerHTML("button_" + id + "AutobuyerToggle", g[id + "AutobuyerOn"] ? "开" : "关");
			d.innerHTML("span_" + id + "AutobuyerInterval", timeFormat(autobuyerMeta.interval(id)));
			d.innerHTML("button_" + id + "AutobuyerUpgrade", "将间隔减少 " + ((g[id + "AutobuyerUpgrades"] >= autobuyerMeta.softcap(id)) ? "1" : "5") + "%<br>成本：" + autobuyerMeta.cost(id).noLeadFormat(2) + " " + autobuyers[id].extRes);
		...
		if (achievement.ownedInTier(5) >= 6) { d.display("button_lockManualStardustUpgrades", "inline-block"); d.innerHTML("button_lockManualStardustUpgrades", "手动购买星尘升级 " + (g.confirmations.buyStardustUpgrade ? "禁" : "启") + "用"); }
		...
		d.innerHTML("button_stardustAutomatorMode", stardustAutomatorModes[g.stardustAutomatorMode] ?? "星尘数量");
		d.innerHTML("button_wormholeAutomatorMode", wormholeAutomatorModes[g.wormholeAutomatorMode] ?? "霍金辐射数量");
		d.class("button_starAllocatorToggle", "automatortoggle " + (g.starAllocatorOn ? "on" : "off"));
		d.innerHTML("button_starAllocatorToggle", g.starAllocatorOn ? "开" : "关");
		d.class("button_stardustAutomatorToggle", "automatortoggle " + (g.stardustAutomatorOn ? "on" : "off"));
		d.innerHTML("button_stardustAutomatorToggle", g.stardustAutomatorOn ? "开" : "关");
		d.class("button_wormholeAutomatorToggle", "automatortoggle " + (g.wormholeAutomatorOn ? "on" : "off"));
		d.innerHTML("button_wormholeAutomatorToggle", g.wormholeAutomatorOn ? "开" : "关");
		d.innerHTML("button_researchAutobuyerMode", researchAutobuyerModes[g.researchAutobuyerMode] ?? "所有免费研究");
		// 输入存储
	...
	if (g.activeSubtabs.wormhole === "research") {
		...
		for (let i = 0; i < [0, 3, 6, 8, 9][lightTiersUnlocked()]; i++) {
			...
			if (g.achievement[815] && g.ach815RewardActive) {
				...
				if (lightComponents(i) === null) { d.innerHTML("button_" + name + "ChromaGen", ((g.activeChroma === i) ? "停止生成" : "生成") + " " + stat.chromaPerSec.format(2) + " " + lightNames[i] + " 色素每秒"); }
				else { d.innerHTML("button_" + name + "ChromaGen", ((g.activeChroma === i) ? "停止转换" : "转换") + " " + stat.chromaPerSec.mul(chromaCostFactor(i)).format(2) + " " + lightComponents(i).map(x => lightNames[x]).joinWithAnd() + " 色素为 " + stat.chromaPerSec.format(2) + " " + lightNames[i] + " 色素每秒"); }
			}
		}
		...
		if (lightTiersUnlocked() > 1) {
			...
			d.innerHTML("button_reviewYellowLight0", g.showLightEffectsFrom0 ? "查看当前影响" : "查看下一个效果");
		...
		for (let i = 1; i < galaxyEffects.length; i++) {
			if (g.highestGalaxies + 1 < galaxyEffects[i].req) {
				tooltip = "下一对效果在 " + N(galaxyEffects[i].req).sub(Decimal.max(effectiveGalaxies.add(false), effectiveGalaxies.add(true)).ceil()).sub(c.d1).format() + " 个星系";
				break;
			}
		}
		...
		for (let upg of prismaticUpgradeList) {
			...
			if (prismaticUpgradeUnlocked(upg)) {
				...
				d.innerHTML("span_prismaticUpgrade_" + upg + "_Cost", Decimal.eq(owned, data.max ?? c.maxvalue) ? "已达上限" : ("成本：" + ((showFormulas && (data.max ?? c.maxvalue).gt(c.d1)) ? formulaFormat(unlimited ? (data.scale.noLeadFormat(2) + "<sup>λ</sup> × " + data.baseCost.format()) : data.costFormula()) : prismaticUpgradeCost(upg, affordable.max(c.d1)).format())));
		...
		if (g.studyCompletions[13] > 23) {
			...
			let buttonText = "获取 " + ((g.antimatter.gte(antimatterGalaxy.req()) && g.antimatterGalaxyBulk) ? (antimatterGalaxy.affordable().sub(g.antimatterGalaxies).format() + " 个反物质星系") : "一个反物质星系");
			if (showFormulas) { buttonText += " (需要 " + antimatterGalaxy.reqFormula() + " 反物质)"; }
			else { buttonText += "<br>(进度" + (g.antimatter.gte(antimatterGalaxy.req()) ? "至下一个" : "") + "：" + g.antimatter.format() + " / " + antimatterGalaxy.req(g.antimatterGalaxyBulk ? antimatterGalaxy.affordable().max(g.antimatterGalaxies) : g.antimatterGalaxies).format() + " 反物质)"; }
			buttonText += "<br><br>" + antimatterGalaxy.effectHTML();
			d.innerHTML("span_antimatterGalaxies", g.antimatterGalaxies.format());
			d.innerHTML("span_realAntimatterGalaxies", Decimal.eq(g.antimatterGalaxies, stat.realAntimatterGalaxies) ? "" : ("实际：" + (Decimal.eq(g.antimatterGalaxies, antimatterGalaxy.affordable()) ? stat.realAntimatterGalaxies.noLeadFormat(3) : arrowJoin(stat.realAntimatterGalaxies.noLeadFormat(3), calcStatWithDifferentBase("realAntimatterGalaxies", antimatterGalaxy.affordable()).noLeadFormat(3)))));
		...
		for (let i = 1; i < 13; i++) {
			...
			d.innerHTML("span_wormholeUpgrade" + i + "Cost", (Math.max(wormholeUpgrades[i].max, 2) === g.wormholeUpgrades[i]) ? "已达上限" : ("成本：" + wormholeUpgrades[i].cost.format(0, [3, 9].includes(i) ? 1 : 0) + " 霍金辐射"));
		}
	} else if (g.activeSubtabs.wormhole === "study13") {
		let p = studyPower(13) - (g.study13Bindings[25] ? 56 : 0), colors; // [边框, 背景, 文字]
		...
		d.innerHTML("span_study13Binding", "从下方的树中选择绑定。每个绑定都会为研究 XIII 添加额外条件，但会增加你的绑定等级。<br>此研究可以批量完成，完成时你的完成次数将增加至你的总绑定等级。");
		...
		let rewardText = [];
		if (nextReward !== Infinity) { rewardText.push("你将在 " + nextReward + " 次完成时解锁一个新奖励"); }
		if (nextUpgrade !== Infinity) { rewardText.push("一个现有奖励将在 " + nextUpgrade + " 次完成时升级"); }
		d.innerHTML("span_study13Reward", (rewardText.length === 0) ? "所有奖励已解锁" : rewardText.join("<br>"));
		if (study13.activeT3 === "bindings") {
			if (selections.study13Binding === undefined) {
				d.innerHTML("study13BindingInfo", "悬停在绑定上以查看更多信息");
			} else {
				let data = study13.bindings[selections.study13Binding];
				d.innerHTML("study13BindingInfo", (study13.bindingPower(selections.study13Binding).eq(c.d1) ? "" : ("<span style=\"float:right;font-size:50%\">" + study13.bindingPower(selections.study13Binding).mul(c.e2).noLeadFormat(3) + "% 已激活</span><br>")) + "<h6 style=\"font-size:16px;margin:0px;\">绑定 " + selections.study13Binding + "</h6><p>" + data.description() + "</p><p>[" + data.lv + " 绑定等级" + ((data.lv === 1) ? "" : "s") + "]</p>" + ((g.study13ShowParentBindings && (data.adjacent_req.length > 0)) ? ("<p>[需要绑定" + ((data.adjacent_req.length === 1) ? "" : "s") + " " + data.adjacent_req.joinWithAnd() + "]</p>") : "") + "<p>" + (g.study13Bindings[selections.study13Binding] ? "<span style=\"color:#ff6666\">(激活)</span>" : "<span style=\"color:#999999\">(未激活)</span>") + "</p>");
			}
			...
			if (study13.rewardSelected === undefined) {
				d.innerHTML("study13RewardInfo", (g.studyCompletions[13] === 0) ? "当你解锁一个奖励时，它会显示在左侧。" : "悬停在奖励上以查看更多信息");
			} else {
				...
				if (data.type === "composite") { out += "<table style=\"text-align:left\"><colgroup><col style=\"width:96px\"><col style=\"width:384px\"></colgroup>" + countTo(currLv).map(x => "<tr style=\"color:" + ((x <= prevLv) ? "inherit" : "var(--binding_light)") + ";padding-bottom:3px;\"><td style=\"vertical-align:top;padding-bottom:3px;\">等级 " + x + "</td><td style=\"vertical-align:top;padding-bottom:3px;\">" + data.desc(x) + "</td></tr>").join("") + "</table>"; }
				else { out += data.desc(currLv, prevLv); }
				out += "<div style=\"position:absolute;bottom:10px;left:10px;font-size:10px;width:calc(100% - 20px);text-align:center;\">" + ((data.type === "single") ? ("等级阈值：" + data.breakpoints[0]) : ("等级阈值：<br>" + data.breakpoints.slice(0, study13.rewardLevels[selected]).map((x, i) => "<div style=\"height:25px;width:60px;display:inline-block;color:" + ((i < prevLv) ? "inherit" : "var(--binding_light)") + "\">等级 " + (i + 1) + "<br>" + x + "</div>").join(""))) + "</div>";
...
function tick(time) { // 游戏循环，由自动运行的函数组成。帧率为 20fps
	g.EMDLevel = EMDLevel();
	if (time < 0) {
		error("发生了一个会导致时间倒流 " + timeFormat(time) + " 的错误");
		return;
	}
	if ((timeState === 0) && timeAlwaysEqualized()) { timeState = 3; } // 在研究 3 和 9 中，除非超频激活，否则时间始终均衡
	if (timeState === 3) {
		let diff = time - 0.05;
		g.dilatedTime += diff;
		time -= diff;
	...
	for (let i of ["timePlayed", "timeThisStardustReset", "timeThisWormholeReset", "timeThisSpacetimeReset", "dilatedTime"]) {
		if ((typeof g[i]) !== "number") {
			g[i] = 0;
			error("<code>" + i + "</code> 已内存泄漏。这是一个已知错误，alemaninc 正在调查");
		}
	}
	g.timePlayed += time;
	...
	if (study13.bound(236) && (g.timeThisWormholeReset > study13.bindingEff(236))) { popup({ text: stat.totalDarkAxis.gt(stat.wormholeDarkAxisReq) ? "你已通过绑定 236 自动完成了研究 XIII。" : ("你因绑定 236 用尽时间而未能完成研究 XIII。<br>你达到了 " + stat.totalDarkAxis.format() + " / " + studies[13].goal().format() + " 暗轴"), buttons: [["关闭", ""]] }); wormholeReset(); }
	updateStats();
	...
	if (newsSupport.newsletter.spamStart < Date.now()) { // 秘密成就 33 "Stat Mark"
		if (Math.random() < (deltatime / 100) * (1 + (Date.now() - newsSupport.newsletter.spamStart) / 1000)) {
			(newsSupport.newsletter.remaining.length === 0) ? newsSupport.newsletter.finalNotify() : notify("<span style=\"border-style:solid;border-radius:5px;border-width:1px;border-color:#000000\" onMousedown=\"newsSupport.newsletter.ask()\">验证</span>", "#009999", "#00ffff");
	...
	if (g.activeStudy !== 0) {
		...
		if (!g.research[studies[g.activeStudy].research]) {
			forceExit = true;
			exitText = "相关研究未拥有 (" + researchOut(studies[g.activeStudy].research) + ")";
		}
		if (forceExit) {
			popup({ text: "你因存在错误而被强制从研究 " + studies[0].roman(g.activeStudy) + " 中移除。抱歉！<br>详情：" + exitText + "<br>请告知 alemaninc。", buttons: [["关闭", ""]] });
			wormholeReset();
			g.activeStudy = 0;
		}
	}
	...
	// 增量器部分 - 放在最后，否则重置无法正常工作
	for (let i = 0; i < energyTypes.length; i++) { // 能量放在前面，让研究 III 更难 :D
	...
	if (achievement.ownedInTier(5) >= 8 && g.stardustAutomatorOn) {
		...
		if (mode === 0) { doReset = stat.pendingstardust.gte(g.stardustAutomatorValue); } // 达到 X 星尘
		else if (mode === 1) { doReset = g.timeThisStardustReset >= Number(g.stardustAutomatorValue); } // 达到 X 秒
		else if (mode === 2) { doReset = stat.pendingstardust.gte(g.stardust.mul(g.stardustAutomatorValue)); } // 当前星尘乘以 X
		else if (mode === 3) { doReset = stat.pendingstardust.gte(g.stardust.pow(g.stardustAutomatorValue)); } // 当前星尘的 X 次幂
		else if (mode === 4) { doReset = stat.pendingstardust.gte((g.previousStardustRuns.last10.length === 0) ? c.d0 : N(g.previousStardustRuns.last10[0].gain).mul(g.stardustAutomatorValue)); } // 上一次星尘乘以 X
		else if (mode === 5) { doReset = stat.pendingstardust.gte((g.previousStardustRuns.last10.length === 0) ? c.d0 : N(g.previousStardustRuns.last10[0].gain).pow(g.stardustAutomatorValue)); } // 上一次星尘的 X 次幂
		else {
			if (achievement.ownedInTier(5) >= 8) { popup({ text: "由于错误，星尘自动化模式已恢复为默认的星尘数量值。" }); }
	...
	if (achievement.ownedInTier(5) >= 12 && g.wormholeAutomatorOn) {
		...
		if (mode === 0) { doReset = stat.pendinghr.gte(g.wormholeAutomatorValue); } // 达到 X 霍金辐射
		else if (mode === 1) { doReset = g.timeThisWormholeReset >= Number(g.wormholeAutomatorValue); } // 达到 X 秒
		else if (mode === 2) { doReset = stat.pendinghr.gte(g.hawkingradiation.mul(g.wormholeAutomatorValue)); } // 当前霍金辐射乘以 X
		else if (mode === 3) { doReset = stat.pendinghr.gte(g.hawkingradiation.pow(g.wormholeAutomatorValue)); } // 当前霍金辐射的 X 次幂
		else if (mode === 4) { doReset = stat.pendinghr.gte((g.previousWormholeRuns.last10.length === 0) ? c.d0 : N(g.previousWormholeRuns.last10[0].gain).mul(g.wormholeAutomatorValue)); } // 上一次霍金辐射乘以 X
		else if (mode === 5) { doReset = stat.pendinghr.gte((g.previousWormholeRuns.last10.length === 0) ? c.d0 : N(g.previousWormholeRuns.last10[0].gain).pow(g.wormholeAutomatorValue)); } // 上一次霍金辐射的 X 次幂
		else {
			if (achievement.ownedInTier(5) >= 12) { popup({ text: "由于错误，虫洞自动化模式已恢复为默认的霍金辐射数量值。" }); }
	...
	if (researchAutobuyerProgress > 1) {
		if (!(achievement.maxForLocks.research.includes(Number(g.achOnProgressBar)) && achievement.locking(g.achOnProgressBar))) {
			let bought = false; // 检查是否购买了任何东西
			if (g.researchAutobuyerMode === 0) { // 免费研究
				...
				while (true) {
					...
					if (buyable.length === 0) { break; } // 如果购买了任何免费研究，可购买研究列表将更新，因此必须重复
					if (Date.now() > clock) { error("无限循环"); break; }
					for (let i of buyable) { if (researchCost(i).eq(c.d0)) { buySingleResearch(researchRow(i), researchCol(i)); } } // 再次检查成本变化的研究
					updateBuyableResearch();
					bought = true;
				}
			} else { g.researchAutobuyerMode = 0; } // 错误检测
	...
	if (g.autosaveIsOn && savecounter > 0) {
		...
		if (Date.now() - lastAutosaveTime >= autosaveInterval) {
			save();
